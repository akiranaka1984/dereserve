        ...
        <div class="wrapper">
            ...
            <div class="articlePanel mgt_30">
                <ul id="" class="article-wrap slider grid fadeUpTrigger3 muuri fadeUp" style="height: 4249px;">
                    ...
                    @foreach($today_attendances->sortByDesc('updated_at') as $attendance)
            ...
            <div class="news_wrap">
                <div class="news_box">
                    <ul class="news_list">
                        ...
                        @php
                        if(!empty($news->companion_id)){
                            $imgPath = '/storage/photos/'.($news->companion_id).'/'.($news->companion->home_image->photo);
                        }else{
                            $imgPath = '/storage/photos/default/not-to-be-published.jpg';
                        }
                        @endphp
                            <li id="{{ $news->id }}" class="news type-post status-publish format-standard hentry category-news">
                                <img src="{{ asset($imgPath) }}" alt="{{ $news->companion->name }}" class="photo">
                                <div class="news-area-wrap">
                                <div class="date-area">
                                    <time datetime="{{ date('Y/m/d', strtotime($news->updated_at)) }}">{{ date('Y/m/d', strtotime($news->updated_at)) }}</time>
